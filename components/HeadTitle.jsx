...
const HeadTitle = ({
  ...
  return (
    <View className="flex-row mx-auto mt-2 justify-between w-11/12 h-[50] ">
      {(srcIconLeft && (
        <TouchableOpacity
          className={`${middleText ? "w-[36%]" : "w-4/12"} `}
          activeOpacity={0.8}
          onPress={() => {
            srcIconLeft === icons.back && router.back();
          }}
        >
      ...
      {titleLeft && (
        <View className="flex-row w-4/12 mt-2">
          <Text className="text-slate-500 text-xl font-semibold">Cart</Text>
      ...
      {srcIconMiddle && (
        <View
          className={`mt-3 ${middle ? "w-[61%] relative" : "w-4/12"} `}
          onPress={() => router.push("/favourite")}
        >
          <Image
            source={srcIconMiddle}
            className={`mx-auto ${middle && "absolute right-0"} `}
          />
        </View>
      )}
      {middleText && (
        <Text className="my-auto text-3xl font-semibold w-full ">
          {middleText}
        </Text>
      )}
      {srcIconRight && (
        <TouchableOpacity
          className="mt-3 w-4/12 flex-row justify-end "
          onPress={() => router.push("/favourite")}
        >
