...
const SignUp = () => {
  ...
  const validate = () => {
    ...
    if (form.password && form.password.split('').length < 8)newErrors.push("The password must be above 8 char");
    // 
  ...
  return (
    <SafeAreaView className='w-full bg-white h-full'>
      ...
      <View className='flex flex-row justify-between w-11/12 mx-auto'>
        <InputForm handleChangeText={(value) => handleChange('firstname', value)} value={form.firstname} name='firstname' placeholder='Khaled'  title='First Name' type='normal' size='half'  error={errors?.includes("The first name is required") ? "The first name is required" : ''}/>
    ...
    <View className={`mx-auto ${errors.length? ' mt-1 ' : ' mt-12 '} flex flex-row`}>

          <Text className='text-lg font-[200]'>
            Already have account?
          </Text>
            <TouchableOpacity onPress={()=>{router.replace('/log-in')}}><Text className='mt-0 text-lg text-secondary font-[600] underline'>
            log in </Text>
              </TouchableOpacity>
    </View>
    </SafeAreaView>
